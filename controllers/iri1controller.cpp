...
#define RELOAD_PRIORITY_2 	 5
#define RALENTIRAR_PRIRITY   6
#define RRRRR 	 7
...
int SPEED = 600;
int apagadas = 0;
int inhibirSeguir = 0; // 0 cuando inhibe, 1 cuando no
int volverActivado = 0;
int econtrado = 0;
int cargar = 0; // 0 cuando no debe cargar, 1 cuando sí (inhabilita los demás comportamiento excepto luz roja)
int seguirCargando = 0; // Se pone a 1 la primera vez que se pone a cargar y  no vuelve a cero hasta que la batería no esté completamente cargada
class node
{
...

{
	...
	m_seLight_Red = (CRealRedLightSensor*) m_pcEpuck->GetSensor(SENSOR_REAL_RED_LIGHT);
	/* Set BLUE light Sensor */
	m_seLight_Blue = (CRealBlueLightSensor*) m_pcEpuck->GetSensor(SENSOR_REAL_BLUE_LIGHT);
	/* Set contact Sensor */
	...
	/* Set battery Sensor */
	m_seBattery_Blue = (CBlueBatterySensor*) m_pcEpuck->GetSensor (SENSOR_BLUE_BATTERY);
	/* Set encoder Sensor */
  ...
  onlineMap[robotStartGridX][robotStartGridY] = NEST;
...
void CIri1Controller::ExecuteBehaviors ( void )
{
  ...
  /* Execute Behaviors */
	...
	SeguirLuces       ( RELOAD_PRIORITY );
	EvitarLuces       ( RELOAD_PRIORITY_2 );
	SinBateria       ( RRRRR );
...
/******************************************************************************/
int cargada = 1;
void CIri1Controller::SinBateria ( unsigned int un_priority )
{
	/* Leer Battery Sensores */
	double* battery = m_seBattery_Blue->GetSensorReading(m_pcEpuck);

	/* Leer Sensores de Luz */
	double* light = m_seLight_Blue->GetSensorReading(m_pcEpuck);
  ...
  /* Normalize angle */
	...
	m_fActivationTable[un_priority][0] = fRepelent;
 	...
 	printf("El valor de batería es: %2.4f y el de cargada: \n", battery[0],cargada);
	/* If battery below a BATTERY_THRESHOLD */
	if ( battery[0] < BATTERY_THRESHOLD || cargada == 0)
	{
    	/* Inibit Forage */
...
void CIri1Controller::SeguirLuces ( unsigned int un_priority )
{
  ...
  /* We call vRepelent to go similar to Obstacle Avoidance, although it is an aproaching vector */
	...
	for ( int i = 0 ; i < m_seProx->GetNumberOfInputs() ; i ++ )
	{
		vIr.x += light[i] * cos ( lightDirections[i] );
		vIr.y += light[i] * sin ( lightDirections[i] );
	...
	/* Calc pointing angle */
	float fRepelent = atan2(vIr.y, vIr.x);
  ...
  if (inhibirSeguir == 1.0 && volverActivado == 0 && cargar == 0)
  {
		printf("Seguimos las luces: %2d\n", inhibirSeguir);
	    m_fActivationTable[un_priority][2] = 1.0;
  }
	...
	FILE* filePosition = fopen("outputFiles/SeguirLuces", "a");
	fprintf(filePosition,"%2.4f %2.4f %2.4f %2.4f\n", m_fTime, m_pcEpuck->GetPosition().x, m_pcEpuck->GetPosition().y, m_pcEpuck->GetRotation());
	fclose(filePosition);

	FILE* fileOutput = fopen("outputFiles/lucesAmarillas", "a");
	fprintf(fileOutput, "%2.4f %2.4f %2.4f %2.4f %2.4f %2.4f %2.4f %2.4f %2.4f %2.4f ", m_fTime, light[0], light[1], light[2], light[3], light[4], light[5], light[6], light[7]);
	fclose(fileOutput);
...
void CIri1Controller::EvitarLuces ( unsigned int un_priority )
{

	double* light_Red = m_seLight_Red->GetSensorReading(m_pcEpuck);
	...
	for ( int i = 0 ; i < m_seProx->GetNumberOfInputs() ; i ++ )
	{
		vIr.x += light_Red[i] * cos ( lightDirections[i] );
		vIr.y += light_Red[i] * sin ( lightDirections[i] );

		if ( light_Red[i] > fMaxLight )
			fMaxLight = light_Red[i];
	}
	...
	if ( fMaxLight > 0.1 )
	{
//		fEvitarToSeguirInhibitor = 0.0;
		inhibirSeguir = 0;
		/* Set Leds to RED */
		m_pcEpuck->SetAllColoredLeds(LED_COLOR_RED);
	    /* Mark Behavior as active */
   		 m_fActivationTable[un_priority][2] = 1.0;
	} else inhibirSeguir = 1;
	FILE* filePosition = fopen("outputFiles/EvitarLuces", "a");
	fprintf(filePosition,"%2.4f %2.4f %2.4f %2.4f\n", m_fTime, m_pcEpuck->GetPosition().x, m_pcEpuck->GetPosition().y, m_pcEpuck->GetRotation());
	fclose(filePosition);
...
void CIri1Controller::Ralentizar ( unsigned int un_priority )
{
	// Leer Sensores de Suelo Memory
	...
	FILE* filePosition = fopen("outputFiles/Ralentizar", "a");
	fprintf(filePosition,"%2.4f %2.4d \n", m_fTime, SPEED);
	fclose(filePosition);
...
void CIri1Controller::ComputeActualCell ( unsigned int un_priority )
{
  ...
  // Cuando se está buscando el 		destino y se ha llegado a él 
  if ( m_nForageStatus == 1 && econtrado == 2)
  {
  ...
  else if ( m_nForageStatus == 0 && econtrado == 1)
  {
    /* Update forage Status */
    m_nForageStatus = 1;
    /* Asumme Path Planning is done */
